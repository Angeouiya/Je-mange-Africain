    ...
    <section className="border-t border-border" aria-labelledby={`fulfillment-${order.id}`}>
      ...
      {!canUpdate ? (
        ...
        <div className="px-5 py-6 sm:px-6">
          ...
          {result ? <div role={result.type === "error" ? "alert" : "status"} className={`mt-4 flex items-center gap-2 border-y px-3 py-3 text-xs font-bold ${result.type === "success" ? "border-burgundy/25 bg-burgundy/5 text-burgundy" : "border-destructive/25 bg-destructive/[0.06] text-destructive"}`}>{saving ? <LoaderCircle className="h-4 w-4 animate-spin" /> : result.type === "success" ? <CheckCircle2 className="h-4 w-4" /> : <LockKeyhole className="h-4 w-4" />}{result.message}</div> : null}
