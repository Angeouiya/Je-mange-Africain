    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_23rem]">
        ...
        <aside className="h-fit bg-charcoal p-5 text-white xl:sticky xl:top-24">
          <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase text-gold">{locale === "fr" ? "Aperçu appareil" : "Device preview"}</p><h3 className="mt-1 text-sm font-black">Notification mobile</h3></div><div className="inline-flex rounded-md bg-white/8 p-1" aria-label={locale === "fr" ? "Langue de l’aperçu" : "Preview language"}>{(["fr", "en"] as const).map((language) => <button key={language} type="button" onClick={() => setPreviewLocale(language)} aria-pressed={previewLocale === language} className={`h-7 rounded px-2 text-[9px] font-black uppercase ${previewLocale === language ? "bg-white text-charcoal" : "text-white/55"}`}>{language}</button>)}</div></div>
