...
export function TeamPermissionList({ permissions, locale, includeEmpty = false }: { permissions: TeamPermissionMap; locale: "fr" | "en"; includeEmpty?: boolean }) {
  ...
  return <div className="divide-y divide-border border-y border-border">{modules.map((module) => {
    ...
    return <div key={module} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 py-3"><div className="flex min-w-0 items-center gap-3"><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-md ${actions.length ? "bg-burgundy/[0.07] text-burgundy" : "bg-muted text-muted-foreground"}`}><ModuleGlyph module={module} /></span><div className="min-w-0"><p className="break-words text-xs font-black leading-4 text-charcoal">{moduleLabel(module, locale)}</p><p className="mt-0.5 text-[9px] text-muted-foreground">{actions.length ? (locale === "fr" ? `${actions.length} autorisation${actions.length === 1 ? "" : "s"}` : `${actions.length} permission${actions.length === 1 ? "" : "s"}`) : (locale === "fr" ? "Aucun accès" : "No access")}</p></div></div><div className="flex max-w-[8rem] flex-wrap justify-end gap-1">{actions.map((action) => <PermissionBadge key={action} action={action} locale={locale} />)}</div></div>;
  })}</div>;
}

export function PermissionBadge({ action, locale }: { action: AdminAction; locale: "fr" | "en" }) {
  const style = action === "delete" ? "border-destructive/25 bg-destructive/[0.04] text-destructive" : action === "update" ? "border-terre/25 bg-terre/[0.04] text-terre-dark" : action === "create" ? "border-gold/45 bg-gold/[0.10] text-charcoal" : "border-burgundy/20 bg-burgundy/[0.035] text-burgundy";
