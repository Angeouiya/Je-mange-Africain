    ...
    <div className="scroll-pretty flex w-full max-w-full gap-1 overflow-x-auto overscroll-x-contain rounded-lg border border-border bg-white p-1 sm:w-fit" role="tablist" aria-label={label}>
      {items.map((item) => (
        <button
          ...
          className={`flex h-9 min-w-max flex-1 shrink-0 items-center justify-center gap-2 rounded-md px-3 text-xs font-bold transition-colors sm:flex-none ${value === item.value ? "bg-forest text-white" : "text-muted-foreground hover:bg-muted hover:text-charcoal"}`}
        >
          {item.label}
