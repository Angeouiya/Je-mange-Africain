...
function Metric({ position, icon: Icon, label, value, change, kind, locale, detail }: { position: number; icon: typeof TrendingUp; label: string; value: string; change?: number | null; kind: "revenue" | "cost" | "margin" | "rate"; locale: "fr" | "en"; detail?: string }) {
  ...
  const iconStyle = kind === "revenue" ? "bg-terre text-white" : kind === "cost" ? "bg-gold/20 text-charcoal" : kind === "margin" ? "bg-forest/10 text-forest" : "bg-terre/10 text-terre";
  return (
...
function TrendBadge({ change, favourable, locale }: { change?: number | null; favourable: boolean | null; locale: "fr" | "en" }) {
  ...
  return <span className={`inline-flex h-6 items-center gap-1 rounded px-2 text-[9px] font-black ${favourable ? "bg-forest/[0.07] text-forest" : "bg-destructive/[0.07] text-destructive"}`}><Icon className="h-3 w-3" />{change > 0 ? "+" : ""}{formatNumber(change, locale)} %</span>;
}
...
function BridgeFact({ icon: Icon, label, value }: { icon: typeof ShieldCheck; label: string; value: string }) {
  return <div className="flex min-w-0 items-start gap-3 p-4 sm:p-5"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-forest/[0.07] text-forest"><Icon className="h-4 w-4" /></span><div className="min-w-0"><p className="text-[9px] font-bold leading-4 text-muted-foreground">{label}</p><p className="mt-1 truncate text-xs font-black text-charcoal">{value}</p></div></div>;
}
