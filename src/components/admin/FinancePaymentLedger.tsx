    ...
    <div className="space-y-5">
      ...
      <section className="flex flex-col gap-3 border-y border-charcoal/8 bg-[#F8F7F4] px-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-4" aria-label={isFr ? "Période du registre financier" : "Financial ledger period"} data-testid="payment-ledger-period">
        <div className="flex min-w-0 items-start gap-3">
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-burgundy/12 bg-burgundy/[0.08] text-burgundy shadow-[0_12px_26px_-22px_rgba(90,38,50,0.72)]"><ReiconGlyph icon={Clock3} weight="Filled" className="h-4 w-4" /></span>
        ...
        <div className="flex min-w-0 items-start gap-2 sm:items-center">
          {request.loading && request.data ? <span role="status" aria-label={isFr ? "Actualisation" : "Refreshing"}><ReiconGlyph icon={Loader} className="mt-2 h-4 w-4 animate-spin text-terre" /></span> : null}
      ...
      <section className="grid grid-cols-2 overflow-hidden rounded-lg border border-charcoal/8 bg-white xl:grid-cols-4" aria-label={isFr ? "Santé des encaissements" : "Payment health"}>
        <PaymentMetric position={0} icon={DollarCircle} label={isFr ? "Net encaissé" : "Net collected"} value={formatPrice(summary.netCapturedAmount, locale)} detail={`${paymentCountLabel(summary.capturedCount, locale)} · ${formatNumber(summary.reconciliationRate, locale)} % ${isFr ? "rapprochés" : "reconciled"}`} tone="earth" />
        ...
        <PaymentMetric position={2} icon={DollarReload} label={isFr ? "Remboursé" : "Refunded"} value={formatPrice(summary.refundedAmount, locale)} detail={`${summary.refundCount} ${isFr ? "décision(s) tracée(s)" : "audited decision(s)"}${summary.pendingRefundAmount > 0 ? ` · ${formatPrice(summary.pendingRefundAmount, locale)} ${isFr ? "en cours" : "pending"}` : ""}`} tone="burgundy" />
      ...
      <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row">
          ...
          <Button type="button" variant="outline" size="sm" onClick={() => downloadPaymentsCsv(payments, locale)} disabled={!payments.length} className="h-10 shrink-0 border-charcoal/12"><ReiconGlyph icon={Download} className="mr-1.5 h-4 w-4" />{isFr ? "Exporter la page" : "Export page"}</Button>
        </div>
      </div>
  ...
  return (
    <nav className="flex items-center justify-between gap-3 border-t border-charcoal/8 pt-3" aria-label={isFr ? "Pagination du registre financier" : "Financial ledger pagination"} data-testid="payment-pagination">
      ...
      <div className="flex shrink-0 gap-1.5">
        <Button type="button" variant="outline" size="icon" onClick={onPrevious} disabled={!pagination.hasPrevious} className="h-9 w-9" aria-label={isFr ? "Page précédente" : "Previous page"}><ReiconGlyph icon={ChevronLeft} className="h-4 w-4" /></Button>
        <Button type="button" variant="outline" size="icon" onClick={onNext} disabled={!pagination.hasNext} className="h-9 w-9" aria-label={isFr ? "Page suivante" : "Next page"}><ReiconGlyph icon={ChevronRight} className="h-4 w-4" /></Button>
      </div>
    </nav>
  );
}
...
function PaymentMetric({ position, icon, label, value, detail, tone }: { position: number; icon: IconFunction; label: string; value: string; detail: string; tone: "earth" | "gold" | "alert" | "burgundy" }) {
  ...
  return <div className={`min-w-0 p-3 sm:p-5 ${position < 2 ? "border-b" : ""} ${position % 2 === 0 ? "border-r" : ""} border-charcoal/8 xl:border-b-0 ${position < 3 ? "xl:border-r" : "xl:border-r-0"}`}><span className={`grid h-9 w-9 place-items-center rounded-md ${style}`}><ReiconGlyph icon={icon} weight="Filled" className="h-4 w-4" /></span><p className="mt-3 truncate text-lg font-black tabular-nums text-charcoal sm:text-2xl">{value}</p><p className="mt-1 text-xs font-bold text-charcoal">{label}</p><p className="mt-1 text-[9px] leading-4 text-muted-foreground">{detail}</p></div>;
}
...
function PaymentMethodIcon({ method, className = "h-8 w-8" }: { method: string; className?: string }) {
  ...
  const icon = family === "card" ? CreditCard : family === "wallet" ? Mobile : family === "bank" ? Bank : family === "deferred" ? Clock3 : Wallet;
  const tone = family === "bank" ? "border-burgundy/16 bg-burgundy/[0.08] text-burgundy" : family === "wallet" ? "border-gold/30 bg-gold/[0.16] text-burgundy" : "border-terre/14 bg-terre/[0.08] text-terre";
  return <span className={`grid shrink-0 place-items-center rounded-md border ${tone} ${className}`}><ReiconGlyph icon={icon} weight={family === "other" ? "Outline" : "Filled"} className="h-4 w-4" /></span>;
}
