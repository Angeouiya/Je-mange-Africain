...
function AuditMetric({ position, icon: Icon, label, value, detail, tone }: { position: number; icon: typeof ScrollText; label: string; value: string; detail: string; tone: "earth" | "alert" | "burgundy" | "gold" }) {
  const style = tone === "earth" ? "bg-terre text-white" : tone === "alert" ? "bg-destructive/10 text-destructive" : tone === "burgundy" ? "bg-burgundy/10 text-burgundy" : "bg-gold/25 text-charcoal";
...
function ChangeValue({ label, value, after = false }: { label: string; value: string | null; after?: boolean }) { return <div className={`min-w-0 rounded-md px-3 py-2 ${after ? "bg-burgundy/[0.05]" : "bg-muted/60"}`}><p className="text-[8px] font-black uppercase text-muted-foreground">{label}</p><p className="mt-1 break-words text-[10px] leading-4 text-charcoal">{value ?? "—"}</p></div>; }
...
function RiskBadge({ risk = "routine", locale }: { risk?: AuditEntry["risk"]; locale: "fr" | "en" }) { return <Badge variant="outline" className={`h-5 text-[8px] ${risk === "critical" ? "border-destructive/25 bg-destructive/[0.04] text-destructive" : risk === "attention" ? "border-gold/45 bg-gold/[0.08] text-charcoal" : "border-burgundy/20 bg-burgundy/[0.035] text-burgundy"}`}>{riskLabel(risk || "routine", locale)}</Badge>; }
function riskStyle(risk?: AuditEntry["risk"]) { return risk === "critical" ? "bg-destructive/10 text-destructive" : risk === "attention" ? "bg-gold/25 text-charcoal" : "bg-burgundy/10 text-burgundy"; }
