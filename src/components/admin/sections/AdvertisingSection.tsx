...
function Metric({ value, label }: { value: number; label: string }) { return <div><p className="text-2xl font-black tabular-nums text-gold">{value}</p><p className="mt-1 text-[10px] text-white/50">{label}</p></div>; }
...
function AdField({ label, children }: { label: string; children: React.ReactNode }) { return <div className="space-y-1.5"><Label>{label}</Label>{children}</div>; }
