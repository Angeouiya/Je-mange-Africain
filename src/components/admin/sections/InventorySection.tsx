...
import { AlertTriangle, Boxes, CalendarClock, Search, Snowflake, Warehouse } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <label className="relative block w-full lg:max-w-sm"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} className="h-10 bg-white pl-9" placeholder={isFr ? "Produit, lot ou entrepôt" : "Product, batch or warehouse"} /></label>
      </div>

      {filteredBatches.length ? <div className="overflow-hidden rounded-lg border border-black/8 bg-white">
