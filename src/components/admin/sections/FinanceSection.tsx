...
export default function FinanceSection({ locale }: { locale: "fr" | "en" }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-2 overflow-hidden rounded-lg border border-black/8 bg-white lg:grid-cols-[1.25fr_0.75fr_0.75fr]">
        ...
        <div className="border-r border-t border-black/8 p-3 sm:p-5 lg:border-l lg:border-r-0 lg:border-t-0"><span className="grid h-8 w-8 place-items-center rounded-md bg-gold/15 text-amber-700 sm:h-9 sm:w-9"><CreditCard className="h-4 w-4" /></span><p className="mt-3 truncate text-lg font-black tabular-nums sm:mt-4 sm:text-2xl">{formatPrice(pendingAmount, locale)}</p><p className="mt-1 text-[9px] font-bold text-muted-foreground sm:text-[10px]">{isFr ? "en attente" : "pending"}</p></div>
        <div className="border-t border-black/8 p-3 sm:p-5 lg:border-l lg:border-t-0"><span className="grid h-8 w-8 place-items-center rounded-md bg-destructive/10 text-destructive sm:h-9 sm:w-9"><AlertCircle className="h-4 w-4" /></span><p className="mt-3 truncate text-lg font-black tabular-nums sm:mt-4 sm:text-2xl">{formatPrice(exceptionAmount, locale)}</p><p className="mt-1 text-[9px] font-bold text-muted-foreground sm:text-[10px]">{isFr ? "à examiner" : "to review"}</p></div>
      </section>
