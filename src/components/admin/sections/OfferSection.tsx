              ...
              <Table>
                ...
                <TableBody>{filteredProducts.map((product) => (
                  <TableRow key={product.id}>
                    ...
                    <TableCell><Badge variant="outline" className={product.stockQty <= 0 ? "border-destructive/30 bg-destructive/5 text-destructive" : product.stockQty <= (product.alertThreshold || 5) ? "border-gold/40 bg-gold/[0.09] text-charcoal" : "border-forest/25 bg-forest/[0.04] text-forest"}>{product.stockQty <= 0 ? (isFr ? "Rupture" : "Out") : `${product.stockQty} ${isFr ? "en stock" : "in stock"}`}</Badge></TableCell>
