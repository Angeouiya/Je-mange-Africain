...
type Product = { id: string; name: string; traditionalName: string; sku: string; costPrice?: number | null; profitMargin?: number | null; costSource?: "recorded" | "estimated"; price: number; promoPrice?: number | null; stockQty: number; alertThreshold?: number; imageColor: string; imageEmoji: string; imageUrl?: string | null; galleryUrls?: string[]; isNew?: boolean; isRecommended?: boolean; isBestseller?: boolean; status?: string; thermalClass: string; country: string };
              ...
              <Table>
                ...
                <TableBody>{filteredProducts.map((product) => (
                  <TableRow key={product.id}>
                    <TableCell><div className="flex items-center gap-3"><ProductImage src={product.imageUrl || getProductPhoto(product)} alt={product.name} emoji={product.imageEmoji} color={product.imageColor} size="sm" className="h-10 w-10 shrink-0" rounded="rounded-md" /><div className="min-w-0"><p className="truncate text-sm font-extrabold text-charcoal">{product.name}</p><p className="truncate text-[10px] text-muted-foreground">{product.traditionalName}</p><div className="mt-1 flex gap-1">{product.isNew ? <Badge variant="outline" className="h-4 px-1 text-[8px]">{isFr ? "Nouveau" : "New"}</Badge> : null}{product.isRecommended ? <Badge variant="outline" className="h-4 border-forest/25 px-1 text-[8px] text-forest">{isFr ? "Recommandé" : "Recommended"}</Badge> : null}{product.isBestseller ? <Badge variant="outline" className="h-4 border-gold/50 px-1 text-[8px] text-amber-700">{isFr ? "Populaire" : "Popular"}</Badge> : null}</div></div></div></TableCell>
