...
import { ArrowDownRight } from "reicon/icons/ArrowDownRight";
...
const workflowMeta: Record<DashboardPayload["workflow"][number]["id"], { icon: IconFunction; fr: string; en: string; detailFr: string; detailEn: string }> = {
  validate: { icon: ReceiptText, fr: "Qualifier", en: "Qualify", detailFr: "Paiement et réservation", detailEn: "Payment and reservation" },
  prepare: { icon: BoxTick, fr: "Préparer", en: "Prepare", detailFr: "Picking et contrôle", detailEn: "Picking and checks" },
  deliver: { icon: Truck, fr: "Acheminer", en: "Deliver", detailFr: "Transport en cours", detailEn: "In carrier network" },
  closed: { icon: CheckCircle, fr: "Clôturer", en: "Close", detailFr: "Livrées ou terminées", detailEn: "Delivered or completed" },
};

const priorityMeta: Record<Priority["level"], { icon: IconFunction; labelFr: string; labelEn: string; className: string; iconClassName: string }> = {
  ...
  attention: { icon: Timer, labelFr: "À traiter", labelEn: "Action", className: "border-terre/20 bg-terre/[0.035]", iconClassName: "bg-terre/10 text-terre" },
...
function ComparisonBadge({ value, locale }: { value: number | null; locale: Locale }) {
  if (value === null) return <span className="inline-flex h-6 items-center gap-1 rounded bg-gold/15 px-2 text-[9px] font-black text-charcoal"><ReiconGlyph icon={Sparkles} weight="Filled" className="h-3 w-3" />{locale === "fr" ? "Nouvelle base" : "New baseline"}</span>;
  const Icon = value > 0 ? ArrowUpRight : value < 0 ? ArrowDownRight : Minus;
  return <span className={`inline-flex h-6 items-center gap-1 rounded px-2 text-[9px] font-black ${value >= 0 ? "bg-burgundy/[0.07] text-burgundy" : "bg-destructive/[0.07] text-destructive"}`}><ReiconGlyph icon={Icon} weight="Filled" className="h-3 w-3" />{value > 0 ? "+" : ""}{formatNumber(value, locale, 1)} %</span>;
}

function MetricCell({ icon: Icon, label, value, detail, comparison, tone, position, locale }: { icon: IconFunction; label: string; value: string; detail: string; comparison?: number | null; tone: "terre" | "gold" | "burgundy" | "soft"; position: number; locale: Locale }) {
  ...
  return (
    <div className={`min-w-0 p-3.5 sm:p-5 ${position % 2 === 0 ? "border-r border-charcoal/8" : ""} ${position < 2 ? "border-b border-charcoal/8" : ""} ${position < 3 ? "xl:border-r" : "xl:border-r-0"} xl:border-b-0`}>
      <div className="flex items-start justify-between gap-2"><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-md ${iconClass}`}><ReiconGlyph icon={Icon} weight="Filled" className="h-4 w-4" /></span>{comparison !== undefined ? <ComparisonBadge value={comparison} locale={locale} /> : null}</div>
  ...
  return (
    <section className="border border-charcoal/8 bg-white" aria-labelledby="priority-queue-title">
      ...
      {priorities.length ? <div className="divide-y divide-charcoal/8">{priorities.slice(0, 5).map((priority) => {
        ...
        return <button key={priority.id} type="button" onClick={() => onNavigate(priority.target)} className={`group flex w-full items-start gap-3 border-l-2 px-3 py-3 text-left transition hover:bg-terre/[0.035] sm:px-4 ${meta.className}`}><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-md ${meta.iconClassName}`}><ReiconGlyph icon={Icon} weight="Filled" className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="text-xs font-black text-charcoal">{priority.title}</span><span className="text-[8px] font-black uppercase text-muted-foreground">{isFr ? meta.labelFr : meta.labelEn}</span></span><span className="mt-1 block text-[10px] leading-4 text-muted-foreground">{priority.detail}</span></span><span className="mt-0.5 text-base font-black tabular-nums text-charcoal">{priority.count}</span><ReiconGlyph icon={ArrowRight} className="mt-1 h-4 w-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:text-terre" /></button>;
      })}</div> : <div className="flex min-h-52 flex-col items-center justify-center px-6 text-center"><span className="grid h-11 w-11 place-items-center rounded-lg bg-burgundy/[0.07] text-burgundy"><ReiconGlyph icon={CheckCircle} weight="Filled" className="h-5 w-5" /></span><p className="mt-3 text-sm font-black text-charcoal">{isFr ? "Aucune urgence ouverte" : "No open urgency"}</p><p className="mt-1 max-w-xs text-[10px] leading-4 text-muted-foreground">{isFr ? "Les flux critiques sont sous contrôle à cet instant." : "Critical workflows are currently under control."}</p></div>}
    </section>
  );
}
    ...
    <section aria-labelledby="workflow-title">
      ...
      <div className="grid grid-cols-2 overflow-hidden rounded-lg border border-charcoal/8 bg-white lg:grid-cols-4">
        {data.workflow.map((stage, index) => {
          ...
          const Icon = meta.icon;
          return <button key={stage.id} type="button" onClick={() => onNavigate("orders")} className={`group min-w-0 p-4 text-left transition hover:bg-terre/[0.035] ${index % 2 === 0 ? "border-r border-charcoal/8" : ""} ${index < 2 ? "border-b border-charcoal/8" : ""} ${index < 3 ? "lg:border-r" : "lg:border-r-0"} lg:border-b-0`}><div className="flex items-center justify-between gap-2"><span className={`grid h-9 w-9 place-items-center rounded-md ${index === 0 ? "bg-terre/10 text-terre" : index === 1 ? "bg-gold/20 text-charcoal" : index === 2 ? "bg-burgundy/[0.08] text-burgundy" : "bg-muted text-muted-foreground"}`}><ReiconGlyph icon={Icon} weight="Filled" className="h-4 w-4" /></span><span className="text-xl font-black tabular-nums text-charcoal">{stage.count}</span></div><p className="mt-3 text-xs font-black text-charcoal">{isFr ? meta.fr : meta.en}</p><p className="mt-1 text-[9px] text-muted-foreground">{isFr ? meta.detailFr : meta.detailEn}</p></button>;
        })}
      </div>
    </section>
  );
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-charcoal/8 bg-white" aria-labelledby="storefront-mirror-title" data-testid="storefront-mirror">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-charcoal/8 px-4 py-4 sm:px-5">
        ...
        <span className={`inline-flex h-7 items-center gap-1.5 rounded-md px-2.5 text-[9px] font-black ${issueCount ? "bg-gold/20 text-charcoal" : "bg-burgundy/[0.07] text-burgundy"}`}>{issueCount ? <ReiconGlyph icon={AlertTriangle} weight="Filled" className="h-3.5 w-3.5" /> : <ReiconGlyph icon={CheckCircle} weight="Filled" className="h-3.5 w-3.5" />}{issueCount ? `${issueCount} ${isFr ? "point(s) à traiter" : "item(s) to resolve"}` : (isFr ? "Prêt à vendre" : "Ready to sell")}</span>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-2 overflow-hidden rounded-lg border border-charcoal/8 bg-white xl:grid-cols-4" aria-label={isFr ? "Indicateurs de décision" : "Decision metrics"}>
        <MetricCell position={0} locale={locale} icon={DollarCircle} label={isFr ? "Encaissé aujourd'hui" : "Collected today"} value={formatPrice(data.kpis.revenueToday, locale)} detail={isFr ? "paiements nets reconnus" : "recognised net payments"} tone="terre" />
        ...
        <MetricCell position={3} locale={locale} icon={Box} label={isFr ? "Catalogue disponible" : "Available catalogue"} value={`${formatNumber(data.kpis.stockCoverageRate, locale, 1)} %`} detail={`${data.kpis.outOfStock} ${isFr ? "produits indisponibles" : "unavailable products"}`} tone="soft" />
      </section>
      ...
      <div className="grid grid-cols-3 overflow-hidden rounded-lg border border-charcoal/8 bg-white text-center" aria-label={isFr ? "Repères complémentaires" : "Additional signals"}>
        <div className="p-3"><ReiconGlyph icon={Truck} weight="Filled" className="mx-auto h-4 w-4 text-terre" /><strong className="mt-2 block text-sm tabular-nums text-charcoal">{data.kpis.inDelivery}</strong><span className="mt-1 block text-[8px] text-muted-foreground">{isFr ? "en acheminement" : "in delivery"}</span></div>
        <div className="border-x border-charcoal/8 p-3"><ReiconGlyph icon={Users2} weight="Filled" className="mx-auto h-4 w-4 text-burgundy" /><strong className="mt-2 block text-sm tabular-nums text-charcoal">{formatNumber(data.kpis.customers, locale)}</strong><span className="mt-1 block text-[8px] text-muted-foreground">{isFr ? "clients enregistrés" : "registered customers"}</span></div>
        <div className="p-3"><ReiconGlyph icon={ReceiptText} weight="Filled" className="mx-auto h-4 w-4 text-gold" /><strong className="mt-2 block text-sm tabular-nums text-charcoal">{data.kpis.paymentAttention}</strong><span className="mt-1 block text-[8px] text-muted-foreground">{isFr ? "paiements à revoir" : "payments to review"}</span></div>
      </div>
    </div>
  );
}
