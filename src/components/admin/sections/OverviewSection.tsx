...
function ComparisonBadge({ value, locale }: { value: number | null; locale: Locale }) {
  ...
  return <span className={`inline-flex h-6 items-center gap-1 rounded px-2 text-[9px] font-black ${value >= 0 ? "bg-forest/[0.07] text-forest" : "bg-destructive/[0.07] text-destructive"}`}><Icon className="h-3 w-3" />{value > 0 ? "+" : ""}{formatNumber(value, locale, 1)} %</span>;
}
...
function MetricCell({ icon: Icon, label, value, detail, comparison, tone, position, locale }: { icon: LucideIcon; label: string; value: string; detail: string; comparison?: number | null; tone: "terre" | "gold" | "burgundy" | "soft"; position: number; locale: Locale }) {
  const iconClass = tone === "terre" ? "bg-terre text-white" : tone === "gold" ? "bg-gold text-charcoal" : tone === "burgundy" ? "bg-forest text-white" : "bg-terre/[0.08] text-terre";
  return (
...
function PriorityQueue({ priorities, locale, onNavigate }: { priorities: Priority[]; locale: Locale; onNavigate: (section: AdminSectionId) => void }) {
  ...
  return (
    <section className="border border-charcoal/8 bg-white" aria-labelledby="priority-queue-title">
      ...
      })}</div> : <div className="flex min-h-52 flex-col items-center justify-center px-6 text-center"><span className="grid h-11 w-11 place-items-center rounded-lg bg-forest/[0.07] text-forest"><CheckCircle2 className="h-5 w-5" /></span><p className="mt-3 text-sm font-black text-charcoal">{isFr ? "Aucune urgence ouverte" : "No open urgency"}</p><p className="mt-1 max-w-xs text-[10px] leading-4 text-muted-foreground">{isFr ? "Les flux critiques sont sous contrôle à cet instant." : "Critical workflows are currently under control."}</p></div>}
    </section>
  );
}
    ...
    <section aria-labelledby="workflow-title">
      ...
      <div className="grid grid-cols-2 overflow-hidden rounded-lg border border-charcoal/8 bg-white lg:grid-cols-4">
        {data.workflow.map((stage, index) => {
          ...
          const Icon = meta.icon;
          return <button key={stage.id} type="button" onClick={() => onNavigate("orders")} className={`group min-w-0 p-4 text-left transition hover:bg-terre/[0.035] ${index % 2 === 0 ? "border-r border-charcoal/8" : ""} ${index < 2 ? "border-b border-charcoal/8" : ""} ${index < 3 ? "lg:border-r" : "lg:border-r-0"} lg:border-b-0`}><div className="flex items-center justify-between gap-2"><span className={`grid h-9 w-9 place-items-center rounded-md ${index === 0 ? "bg-terre/10 text-terre" : index === 1 ? "bg-gold/20 text-charcoal" : index === 2 ? "bg-forest/[0.08] text-forest" : "bg-muted text-muted-foreground"}`}><Icon className="h-4 w-4" /></span><span className="text-xl font-black tabular-nums text-charcoal">{stage.count}</span></div><p className="mt-3 text-xs font-black text-charcoal">{isFr ? meta.fr : meta.en}</p><p className="mt-1 text-[9px] text-muted-foreground">{isFr ? meta.detailFr : meta.detailEn}</p></button>;
        })}
      </div>
    </section>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 overflow-hidden rounded-lg border border-charcoal/8 bg-white text-center" aria-label={isFr ? "Repères complémentaires" : "Additional signals"}>
        ...
        <div className="border-x border-charcoal/8 p-3"><UsersRound className="mx-auto h-4 w-4 text-forest" /><strong className="mt-2 block text-sm tabular-nums text-charcoal">{formatNumber(data.kpis.customers, locale)}</strong><span className="mt-1 block text-[8px] text-muted-foreground">{isFr ? "clients enregistrés" : "registered customers"}</span></div>
