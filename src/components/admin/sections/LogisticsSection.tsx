...
import { Flash as ReFlash } from "reicon/icons/Flash";
import { Gauge as ReGauge } from "reicon/icons/Gauge";
import { MapPoint as ReMapPoint } from "reicon/icons/MapPoint";
import { RouteTrack as ReRouteTrack } from "reicon/icons/RouteTrack";
import { ShieldCheck as ReShieldCheck } from "reicon/icons/ShieldCheck";
import { Snowflake as ReSnowflake } from "reicon/icons/Snowflake";
import { Star as ReStar } from "reicon/icons/Star";
import { TruckFast as ReTruckFast } from "reicon/icons/TruckFast";
import { TruckTick as ReTruckTick } from "reicon/icons/TruckTick";
...
const SERVICE_PRESENTATION: Record<DeliveryService, { icon: IconFunction; fr: string; en: string; detailFr: string; detailEn: string; accent: string }> = {
  standard: { icon: ReTruckFast, fr: "Standard", en: "Standard", detailFr: "Adresse et chaîne du froid", detailEn: "Doorstep and cold chain", accent: BRAND_COLORS.burgundy },
  express: { icon: ReFlash, fr: "Express", en: "Express", detailFr: "Prioritaire jusqu'à 24 h", detailEn: "Priority within 24 hours", accent: BRAND_COLORS.terracotta },
  relay: { icon: ReMapPoint, fr: "Point relais", en: "Collection point", detailFr: "Produits ambiants uniquement", detailEn: "Ambient goods only", accent: BRAND_COLORS.gold },
};
...
export default function LogisticsSection({ locale, canCreate, canUpdate, canDelete }: { locale: "fr" | "en"; canCreate: boolean; canUpdate: boolean; canDelete: boolean }) {
  ...
  const tabs: Array<{ value: LogisticsTab; label: string; description: string; icon: IconFunction; accent: string }> = [
    { value: "routes", icon: ReRouteTrack, label: isFr ? "Zones tarifaires" : "Rate zones", description: isFr ? "Pays, prix et délais" : "Countries, prices and timing", accent: BRAND_COLORS.earth },
    { value: "carriers", icon: ReTruckTick, label: isFr ? "Transporteurs" : "Carriers", description: isFr ? "Partenaires et suivi" : "Partners and tracking", accent: BRAND_COLORS.burgundy },
    { value: "simulator", icon: ReGauge, label: isFr ? "Simulateur client" : "Customer simulator", description: isFr ? "Contrôler la promesse" : "Verify the promise", accent: BRAND_COLORS.gold },
  ];
  ...
  return (
    <div data-testid="logistics-workspace" className="space-y-4 sm:space-y-5">
      <AdminPageHeader
        ...
        signals={[
          { label: isFr ? "Pays" : "Countries", value: String(data.summary.countries), icon: <ReiconGlyph icon={ReMapPoint} weight="Filled" className="h-3.5 w-3.5" />, tone: "earth" },
...
function LogisticsCommandRail({ data, locale }: { data: LogisticsPayload; locale: "fr" | "en" }) {
  const isFr = locale === "fr";
  const fastestDelay = data.zones.length ? Math.min(...data.zones.map((zone) => zone.minDelayHours)) : 0;
  const averageRating = data.carriers.length ? data.carriers.reduce((sum, carrier) => sum + carrier.rating, 0) / data.carriers.length : 0;
  const coldCoverage = data.summary.routes ? Math.round((data.summary.coldChainRoutes / data.summary.routes) * 100) : 0;
  const mix: DeliveryService[] = ["standard", "express", "relay"];
  const metrics = [
    {
      icon: ReRouteTrack,
      value: String(data.summary.routes),
      label: isFr ? "routes vendables" : "sellable routes",
      detail: isFr ? `${data.summary.countries} pays configuré(s)` : `${data.summary.countries} configured countries`,
      accent: BRAND_COLORS.earth,
    },
    {
      icon: ReSnowflake,
      value: `${coldCoverage} %`,
      label: isFr ? "couverture froid" : "cold coverage",
      detail: isFr ? `${data.summary.coldChainRoutes} route(s) acceptent frais/surgelé` : `${data.summary.coldChainRoutes} chilled/frozen route(s)`,
      accent: BRAND_COLORS.chilli,
    },
    {
      icon: ReTruckTick,
      ...
      <div className="flex flex-col gap-3 px-3.5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[9px] font-black uppercase text-terre">{isFr ? "Miroir livraison client" : "Customer delivery mirror"}</p>
            <h2 id="logistics-command-rail-title" className="mt-1 text-sm font-black text-charcoal">{isFr ? "Ce que le paiement peut promettre maintenant" : "What checkout can promise now"}</h2>
            <p className="mt-1 max-w-2xl text-[10px] leading-4 text-muted-foreground">{isFr ? "Chaque route alimente le panier, la facture, le suivi et les alertes de préparation." : "Each route feeds the basket, invoice, tracking and fulfilment alerts."}</p>
          </div>
        </div>
        <div className="flex gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" role="list" tabIndex={0} aria-label={isFr ? "Mix des services de livraison" : "Delivery service mix"}>
          {mix.map((service) => {
            const presentation = SERVICE_PRESENTATION[service];
            const count = data.summary.serviceCounts[service] || 0;
            return (
              <span key={service} role="listitem" className="inline-flex min-h-9 shrink-0 items-center gap-2 rounded-md border border-charcoal/8 bg-white px-2.5 py-1.5">
                <span className="grid h-6 w-6 place-items-center rounded" style={{ color: presentation.accent, backgroundColor: `${presentation.accent}12` }}><ReiconGlyph icon={presentation.icon} weight="Filled" className="h-3.5 w-3.5" /></span>
                <span className="text-[9px] font-black text-charcoal">{isFr ? presentation.fr : presentation.en}</span>
                <span className="rounded bg-burgundy/[0.07] px-1.5 py-0.5 text-[8px] font-black tabular-nums text-burgundy">{count}</span>
              </span>
            );
          })}
        </div>
...
function RouteRow({ zone, locale, canUpdate, canDelete, onEdit, mutate }: { zone: DeliveryZone; locale: "fr" | "en"; canUpdate: boolean; canDelete: boolean; onEdit: () => void; mutate: Mutate }) {
  ...
  return <article className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 py-3.5 sm:grid-cols-[auto_minmax(12rem,1fr)_minmax(10rem,.55fr)_auto]" data-testid={`delivery-zone-${zone.id}`}>
    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-md" style={{ color: service.accent, backgroundColor: `${service.accent}12` }}><ReiconGlyph icon={Icon} weight="Filled" className="h-[1.1rem] w-[1.1rem]" /></span>
...
function ShippingSimulator({ locale }: { locale: "fr" | "en" }) {
  ...
  const postalValidation = validateEuropeanPostalCode(country, postalCode);

  const simulate = async (event: FormEvent) => {
    event.preventDefault();
    if (!postalValidation.valid) return;
    setStatus("busy"); setError("");
    const response = await fetch("/api/shipping/quote", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ country, postalCode: postalValidation.normalized, weightGrams: Math.round(weightKg * 1000), thermalClasses: [thermal], service: "standard", locale }) }).catch(() => null);
