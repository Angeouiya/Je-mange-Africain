...
export function MediaUploadField({
  ...
  return (
    <div className="space-y-2">
      ...
      <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,image/avif" className="sr-only" aria-label={locale === "fr" ? `Choisir un fichier pour ${label}` : `Choose a file for ${label}`} onChange={(event) => void upload(event.target.files?.[0])} />
