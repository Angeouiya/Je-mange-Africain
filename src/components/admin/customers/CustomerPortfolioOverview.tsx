...
export function CustomerPortfolioOverview({ summary, locale }: { summary: AdminCustomerPortfolioPayload["summary"]; locale: Locale }) {
  ...
  return (
    <section tabIndex={0} className="flex snap-x snap-mandatory overflow-x-auto rounded-lg border border-charcoal/8 bg-white overscroll-x-contain outline-none [scrollbar-width:none] focus-visible:ring-2 focus-visible:ring-burgundy/35 focus-visible:ring-offset-2 xl:grid xl:grid-cols-4 xl:overflow-hidden [&::-webkit-scrollbar]:hidden" aria-label={isFr ? "Santé du portefeuille client" : "Customer portfolio health"}>
      {metrics.map((metric, index) => <div key={metric.label} className={`min-w-[10.5rem] shrink-0 snap-start p-3.5 sm:min-w-[12rem] sm:p-5 xl:min-w-0 ${index < metrics.length - 1 ? "border-r border-charcoal/8" : ""}`}><span className={`grid h-9 w-9 place-items-center rounded-md ${metric.iconClass}`}><metric.icon className="h-4 w-4" /></span><p className="mt-3 truncate text-xl font-black tabular-nums text-charcoal sm:text-2xl">{metric.value}</p><p className="mt-1 text-[11px] font-extrabold text-charcoal sm:text-xs">{metric.label}</p><p className="mt-1 text-[9px] leading-4 text-muted-foreground">{metric.detail}</p></div>)}
    </section>
  );
}

export function CustomerPortfolioComposition({ summary, locale }: { summary: AdminCustomerPortfolioPayload["summary"]; locale: Locale }) {
  const isFr = locale === "fr";
