...
export function InviteMemberDialog({ locale, roles, onInvited }: { locale: "fr" | "en"; roles: TeamRole[]; onInvited: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={handleOpen}>
    <DialogTrigger asChild><Button size="sm" className="h-10 bg-forest text-white hover:bg-forest-dark"><MailPlus className="mr-1.5 h-4 w-4" />{isFr ? "Inviter un membre" : "Invite member"}</Button></DialogTrigger>
    <DialogContent closeLabel={isFr ? "Fermer" : "Close"} className="max-h-[calc(100svh-1rem)] overflow-hidden p-0 sm:max-w-3xl">
      <form onSubmit={submit} className="flex max-h-[calc(100svh-1rem)] min-h-0 flex-col">
        <DialogHeader className="shrink-0 border-b border-border px-5 py-5 pr-14 text-left sm:px-6"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-forest text-white"><MailPlus className="h-4 w-4" /></span><div><DialogTitle>{isFr ? "Créer un accès professionnel" : "Create professional access"}</DialogTitle><DialogDescription className="mt-1">{isFr ? "L'invitation attribue un rôle précis, vérifié à chaque action par le serveur." : "The invitation assigns a precise role, verified by the server on every action."}</DialogDescription></div></div></DialogHeader>
        ...
        <DialogFooter className="shrink-0 border-t border-border bg-white px-5 py-4 sm:px-6"><Button type="button" variant="outline" onClick={() => handleOpen(false)} disabled={busy}>{isFr ? "Annuler" : "Cancel"}</Button><Button type="submit" disabled={!complete || busy} className="bg-forest text-white hover:bg-forest-dark">{busy ? <LoaderCircle className="mr-1.5 h-4 w-4 animate-spin" /> : <MailPlus className="mr-1.5 h-4 w-4" />}{isFr ? "Envoyer l'invitation" : "Send invitation"}</Button></DialogFooter>
      </form>
    </DialogContent>
  </Dialog>;
}
...
export function MemberAccessDialog({ member, roles, locale, onClose, onUpdated }: { member: TeamMember; roles: TeamRole[]; locale: "fr" | "en"; onClose: () => void; onUpdated: () => void }) {
  ...
  return <Dialog open onOpenChange={(next) => { if (!next && !busy) onClose(); }}>
    <DialogContent closeLabel={isFr ? "Fermer" : "Close"} className="flex max-h-[calc(100svh-1rem)] min-h-0 flex-col gap-0 overflow-hidden p-0 sm:max-w-3xl">
      <DialogHeader className="shrink-0 border-b border-border px-5 py-5 pr-14 text-left sm:px-6"><div className="flex items-start gap-3"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-md ${protectedAccount ? "bg-charcoal text-white" : "bg-forest text-white"}`}><UserRoundCog className="h-4 w-4" /></span><div className="min-w-0"><p className="text-[9px] font-black uppercase text-terre">{isFr ? "Identité professionnelle" : "Professional identity"}</p><DialogTitle className="mt-1 truncate">{name}</DialogTitle><DialogDescription className="mt-1 break-all">{member.email}</DialogDescription></div></div></DialogHeader>
