    ...
    >
      <div className="relative">
        ...
        <button
          ...
          className={`absolute right-2 top-2 grid place-items-center border border-black/8 bg-white/90 shadow-sm backdrop-blur transition hover:bg-white ${compact ? "h-7 w-7 rounded-md" : "h-8 w-8 rounded-full"}`}
        >
      ...
      <div className={`flex min-w-0 flex-1 flex-col ${compact ? "gap-1 p-2" : "gap-2.5 p-3.5"}`}>
        ...
        <div>
          ...
          <p className={`${compact ? "text-[9px]" : "text-[11px]"} mt-0.5 line-clamp-1 font-medium text-terre`}>{product.traditionalName}</p>
        </div>
