...
export function StorefrontAdvertisementArtwork({ advertisement, placement, variant = "ribbon", locale, className = "", edgeToEdgeMobile = false, showAction = false, onActivate, requiresSignIn = false, actionDisabled = false, testId }: ArtworkProps) {
  ...
  const actionLabel = requiresSignIn ? labels.signIn : labels.action;
    ...
    >
      ...
      <div className={`relative flex flex-col items-start justify-center ${isImmersive ? "min-h-48 justify-end p-5 text-white md:min-h-64 md:justify-center md:p-9" : isCompact ? "min-h-[7.5rem] max-w-[84%] p-3.5 sm:max-w-[70%] sm:p-4" : "min-h-[9.5rem] max-w-[86%] p-4 sm:max-w-[68%] sm:p-5"}`}>
        ...
        {showAction ? onActivate ? (
          <Button type="button" onClick={onActivate} disabled={actionDisabled} className={actionClassName}>{actionLabel}<ArrowRight className="ml-1 h-3.5 w-3.5" /></Button>
        ) : (
          <span aria-hidden="true" className={`inline-flex items-center justify-center rounded-md font-semibold ${actionClassName}`}>{actionLabel}<ArrowRight className="ml-1 h-3.5 w-3.5" /></span>
        ) : null}
      </div>
    </section>
  );
...
export function StorefrontAdvertisement({ placement, variant = "ribbon", fallback, fallbackDestination, className = "" }: Props) {
  ...
  const openDestination = () => {
    ...
    if (!customer) {
      if (destination?.kind === "storefront") requestCustomerAuth({ view: destination.view, params: destination.params });
      else if (fallbackDestination) requestCustomerAuth({ view: fallbackDestination.view, params: fallbackDestination.params || {} });
      else requestCustomerAuth({ view: "home", params: {} });
      return;
    }
