...
type Props = {
  placement: AdvertisementPlacement;
  variant?: "immersive" | "ribbon" | "compact";
...
export function StorefrontAdvertisement({ placement, variant = "ribbon", fallback, fallbackDestination, className = "" }: Props) {
  ...
  return (
    <section
      ...
      data-testid={`advertisement-${placement}`}
      data-advertisement-id={advertisement.id}
      className={`relative isolate overflow-hidden border border-burgundy/10 ${isImmersive ? "-mx-4 min-h-48 md:mx-0 md:min-h-64 md:rounded-lg" : isCompact ? "min-h-[7.5rem] rounded-lg" : "min-h-[9.5rem] rounded-lg"} ${className}`}
    >
      <Image
      ...
      <div className={`relative flex flex-col items-start justify-center ${isImmersive ? "min-h-48 justify-end p-5 text-white md:min-h-64 md:justify-center md:p-9" : isCompact ? "min-h-[7.5rem] max-w-[84%] p-3.5 sm:max-w-[70%] sm:p-4" : "min-h-[9.5rem] max-w-[86%] p-4 sm:max-w-[68%] sm:p-5"}`}>
        ...
        {(advertisement.linkUrl || fallbackDestination) ? (
          <Button type="button" onClick={openDestination} className={`${isImmersive ? "mt-4 h-9 bg-white px-3 text-burgundy hover:bg-cream md:h-10 md:px-4" : "mt-2.5 h-8 bg-burgundy px-2.5 text-[10px] text-white hover:bg-burgundy/90"}`}>
            {labels.action}<ArrowRight className="ml-1 h-3.5 w-3.5" />
          </Button>
        ) : null}
      </div>
    </section>
