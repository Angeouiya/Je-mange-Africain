...
export function MarketChannelSwitch({ channel }: { channel: "retail" | "wholesale" }) {
  ...
  const warmDestination = (view: "catalog" | "wholesale") => {
    void preloadStorefrontViewBundle(view);
    void prefetchStorefrontData(view, {}, locale);
  };
      ...
      {options.map((option) => {
        ...
        return (
          <button
            key={option.id}
            type="button"
            onPointerEnter={() => warmDestination(option.view)}
            onFocus={() => warmDestination(option.view)}
            onTouchStart={() => warmDestination(option.view)}
            onClick={() => {
              warmDestination(option.view);
              navigate(option.view);
            }}
            aria-pressed={active}
