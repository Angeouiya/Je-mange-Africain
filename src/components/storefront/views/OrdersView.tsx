      ...
      {loading ? (
        ...
        <>
          <section className="mb-5 grid grid-cols-2 border-y border-charcoal/10 sm:grid-cols-4" aria-label={locale === "fr" ? "Synthèse des commandes" : "Order summary"} data-testid="orders-portfolio">
            ...
            <OrderMetric icon={CircleDollarSign} label={locale === "fr" ? "Montant" : "Value"} value={formatPrice(portfolio.orderedValue, locale)} />
          </section>
          ...
          <section className="mb-5 border-b border-border pb-4" aria-label={locale === "fr" ? "Rechercher et filtrer les commandes" : "Search and filter orders"}>
            ...
            <div className="grid grid-cols-2 gap-1.5 min-[390px]:grid-cols-4 sm:gap-2" role="group" aria-label={locale === "fr" ? "Filtrer par statut" : "Filter by status"} data-testid="order-filter-controls">
              {filters.map((item) => <button key={item.id} type="button" aria-pressed={filter === item.id} onClick={() => setFilter(item.id)} className={`inline-flex min-h-10 min-w-0 items-center justify-center gap-1.5 rounded-md border px-2 py-1 text-[10px] font-bold leading-3.5 transition-colors min-[390px]:h-9 min-[390px]:min-h-0 min-[390px]:gap-1 min-[390px]:px-1.5 sm:gap-2 sm:px-3 sm:text-xs ${filter === item.id ? "border-burgundy bg-burgundy text-white" : "border-border bg-white text-muted-foreground hover:border-burgundy/30 hover:text-burgundy"}`}><span data-filter-label className="min-w-0 whitespace-nowrap">{item.label}</span><span className={`min-w-4 shrink-0 rounded px-1 text-center text-[9px] sm:min-w-5 sm:text-[10px] ${filter === item.id ? "bg-white/15 text-white" : "bg-muted text-charcoal"}`}>{counts[item.id]}</span></button>)}
            </div>
            </div>
...
function OrderMetric({ icon: Icon, label, value, className = "", urgent = false }: { icon: typeof Package; label: string; value: string; className?: string; urgent?: boolean }) {
  return <div className={`flex min-w-0 items-center gap-2 border-charcoal/10 px-2.5 py-3.5 min-[390px]:gap-3 min-[390px]:px-3 sm:px-4 ${className}`}><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-md min-[390px]:h-9 min-[390px]:w-9 ${urgent ? "bg-gold/20 text-terre" : "bg-terre/[0.07] text-terre"}`}><Icon className="h-4 w-4" /></span><div className="min-w-0"><p data-order-metric-label className="whitespace-nowrap text-[8px] font-black uppercase text-muted-foreground">{label}</p><p className="mt-0.5 truncate text-sm font-black tabular-nums text-charcoal">{value}</p></div></div>;
}
