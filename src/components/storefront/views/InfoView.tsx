...
function ContactForm({ locale }: { locale: "fr" | "en" }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      {message ? <div role={status === "error" ? "alert" : "status"} className={`flex gap-2 rounded-md border p-3 text-xs ${status === "success" ? "border-forest/25 bg-forest/5 text-forest" : "border-destructive/25 bg-destructive/[0.06] text-destructive"}`}>{status === "success" ? <CheckCircle2 className="h-4 w-4 shrink-0" /> : null}<span>{message}</span></div> : null}
