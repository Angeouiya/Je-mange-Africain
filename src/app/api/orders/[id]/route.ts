...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const products = await db.product.findMany({ where: { id: { in: Array.from(new Set(order.items.map((item) => item.productId))) } }, include: { category: true, translations: true, variants: true } });
  ...
  return NextResponse.json({
    ...
    items: order.items.map((it) => {
      ...
      const currentUnitPrice = product
        ...
        : Number(it.unitPrice);
      return {
        id: it.id, productId: it.productId, variantId: it.variantId, variantLabel: it.variantLabel,
