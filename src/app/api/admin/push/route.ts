import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";
import { authorizeAdminRequest } from "@/lib/admin-auth";
import { broadcastLocalizedPush, getPushAudienceCounts } from "@/lib/push-server";
import { PUSH_AUDIENCES } from "@/lib/push-audience";
import { db } from "@/lib/db";

export const dynamic = "force-dynamic";

const CAMPAIGN_ROLES = new Set(["super_admin", "direction", "marketing", "support"]);
const Campaign = z.object({
  titleFr: z.string().trim().min(3).max(80),
  titleEn: z.string().trim().min(3).max(80),
  bodyFr: z.string().trim().min(8).max(220),
  bodyEn: z.string().trim().min(8).max(220),
  type: z.enum(["promotion", "recipe", "system"]).default("system"),
  url: z.enum(["/", "/?view=catalog", "/?view=recipes", "/?view=orders"]).default("/"),
  audience: z.enum(PUSH_AUDIENCES).default("all"),
});

export async function GET(request: NextRequest) {
  const authorization = await authorizeAdminRequest(request, { module: "marketing", action: "read" });
  if (!authorization.ok) return authorization.response;
  const [audiences, recent] = await Promise.all([
    getPushAudienceCounts(),
    db.notification.findMany({ where: { channel: "push" }, orderBy: { createdAt: "desc" }, take: 8 }),
  ]);
  return NextResponse.json({ activeSubscriptions: audiences.all, audiences, recent });
}

export async function POST(request: NextRequest) {
  const authorization = await authorizeAdminRequest(request, { module: "marketing", action: "create" });
  if (!authorization.ok) return authorization.response;
  if (!CAMPAIGN_ROLES.has(authorization.user.role)) {
    return NextResponse.json({ error: "Votre rôle ne permet pas d'envoyer une campagne." }, { status: 403 });
  }
  const parsed = Campaign.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "Contenu de campagne invalide." }, { status: 400 });

  const { url, audience, ...notificationContent } = parsed.data;
  const campaign = await db.notification.create({
    data: { ...notificationContent, url, audience, channel: "push", sent: false },
  });
  const delivery = await broadcastLocalizedPush({
    fr: { title: parsed.data.titleFr, body: parsed.data.bodyFr, url, type: parsed.data.type, tag: `campaign-${campaign.id}` },
    en: { title: parsed.data.titleEn, body: parsed.data.bodyEn, url, type: parsed.data.type, tag: `campaign-${campaign.id}` },
  }, audience);
  await db.notification.update({
    where: { id: campaign.id },
    data: {
      sent: delivery.sent > 0,
      recipientCount: delivery.total,
      deliveredCount: delivery.sent,
      failedCount: delivery.failed,
    },
  });
  await db.auditLog.create({
    data: {
      action: "push_campaign_sent",
      entityType: "Notification",
      entityId: campaign.id,
      reason: `${delivery.sent}/${delivery.total} appareils notifiés · audience ${audience}`,
      after: JSON.stringify({ ...parsed.data, delivery }),
    },
  });
  return NextResponse.json({ campaign, delivery });
}
